# eg/cross_compile_shared.pl
# Demonstrates cross-compiling Brocken source code into shared libraries
# (.so, .dll) for multiple target operating systems (Linux, Windows, FreeBSD).
use v5.40;
use lib '../lib';
...
for my $target (@targets) {
    ...
    say " -> Building $file";
    my $compiler = Brocken::Compiler->new( type => 'shared', os => $target->{os}, arch => $target->{arch} );
