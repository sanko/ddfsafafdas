    ...
    class Brocken::Target::X64 : isa(Brocken::Target) {
        ...
        method emit_op( $as, $inst, $reg_map, $driver ) {
            ...
            # Inside Brocken::Target::X64 -> emit_op
                ...
                $as->store_mem_disp_reg( $reg_map->{ $inst->{args}[0] }, $inst->{args}[1], $v->( $inst->{args}[2] ) );
            }
                ...
                if ( $idx =~ /^%/ ) {
                    ...
                    $as->store_mem_disp_byte( 'rax', 0, $src );
                    $as->pop_reg('rax');
                }
...
Brocken::Target::X64 - x64 machine code generation for each IR opcode

=head1 DESCRIPTION

Maps every IR instruction to x64 machine code via the X64::Emit low-level encoder. Handles ABI argument passing,
function prologues/epilogues, shadow stack operations, memory addressing, control flow, and arithmetic.

=head1 METHODS

=head2 registers

Returns the list of allocatable callee-saved registers (varies by OS ABI).

=head2 emit_op($as, $inst, $reg_map, $driver)

Dispatch table for all non-intrinsic IR opcodes.

=head2 compile_intrinsic($as, $inst, $reg_map, $driver)

Delegates to the Platform module's intrinsic handler.
