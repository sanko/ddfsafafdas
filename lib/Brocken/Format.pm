package Brocken::Format {
    ...
    class Brocken::Format::PE : isa(Brocken::Format) {
        ...
        method write_bin( $filename, $text, $data, $arch, $os ) {
            ...
            print $fh pack(
                ...
                0x20B,                   14, 0, $l->get('.text')->{size}, $l->get('.data')->{size} + length($idata_raw), 0, $l->get('.text')->{rva},
                $l->get('.text')->{rva}, 0x140000000, 0x1000, 0x200,      6, 0, 0, 0, 6, 0, 0, $image_size, $l->header_size, 0, 3, 0x8140, 0x100000,
    ...
    class Brocken::Format::ELF : isa(Brocken::Format) {
        ...
        method write_bin( $f, $text, $data, $arch, $os ) {
            ...
            # Layout is already calculated during pre_layout.
            my $base = 0x400000;
            my $ehdr = pack(
            ...
            for my $s ( $l->sections ) {
                ...
                if ( length($payload) < $s->{size} ) { $payload .= ( "\0" x ( $s->{size} - length($payload) ) ); }
                seek( $fh, $s->{off}, 0 );
                print $fh $payload;
            }
            close $fh;
