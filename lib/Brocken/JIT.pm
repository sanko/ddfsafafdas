...
Brocken::JIT - Just-In-Time compilation engine

=head1 SYNOPSIS

  my $jit = Brocken::JIT->new();
  my $val = $jit->compile_and_run("say 123; return 456;");

=head1 DESCRIPTION

Compiles Brocken source code directly into executable memory and executes it. Coordinates Lexer, Parser, Lowering,
Optimizer, and the architecture-specific JIT assembler.

=head1 METHODS

=head2 compile_and_run($source)

Lexes, parses, lowers, optimizes, and JITs the C<$source>. Executes the result and returns the return value.

=head2 compile_source($source)

Compiles C<$source> and returns a hashref containing C<addr> (the pointer to executable code).
