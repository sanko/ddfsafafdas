package Brocken::Emit {
    ...
    class Brocken::Emit::ARM64 {
        our %REG = (
            ...
            x13=>13,x14=>14,x15=>15,x16=>16,x17=>17,x18=>18,x19=>19,x20=>20,x21=>21,x22=>22,
            x23=>23,x24=>24,x25=>25,x26=>26,x27=>27,x28=>28,x29=>29,x30=>30,sp=>31,xzr=>31
        );
        field $code : reader = ''; field %labels; field @fixups;
        ...
        method add_reg( $dest, $src ) { my $d = $self->reg($dest); my $s = $self->reg($src); $code .= pack( 'L<', 0x8B000000 | ( $s << 16 ) | ( $d << 5 ) | $d ); }
        method mul_reg( $dest, $src ) { my $d = $self->reg($dest); my $s = $self->reg($src); $code .= pack( 'L<', 0x9B007C00 | ( $s << 16 ) | ( $d << 5 ) | $d ); }
        method cmp_reg_reg( $l, $r ) { my $ld = $self->reg($l); my $rd = $self->reg($r); $code .= pack( 'L<', 0xEB000000 | ( $rd << 16 ) | 31 | ( $ld << 5 ) ); }
        method setcc( $cc, $dest ) { my $d = $self->reg($dest); my $inv = $cc ^ 1; $code .= pack( 'L<', 0x9A9F03E0 | ( $inv << 12 ) | $d ) }
        method test_reg_reg( $l, $r ) { my $ld = $self->reg($l); my $rd = $self->reg($r); $code .= pack( 'L<', 0xEA000000 | ( $rd << 16 ) | 31 | ( $ld << 5 ) ); }
        method load_reg_mem( $dest, $src, $disp = 0 ) { my $d = $self->reg($dest); my $s = $self->reg($src); $code .= pack( 'L<', 0xF9400000 | ( ( $disp >> 3 ) << 10 ) | ( $s << 5 ) | $d ); }
        method load_reg_mem_byte( $dest, $src, $disp = 0 ) { my $d = $self->reg($dest); my $s = $self->reg($src); $code .= pack( 'L<', 0x39400000 | ( $disp << 10 ) | ( $s << 5 ) | $d ); }
        method store_mem_disp_reg( $base, $disp, $src ) { my $b = $self->reg($base); my $s = $self->reg($src); $code .= pack( 'L<', 0xF9000000 | ( ( $disp >> 3 ) << 10 ) | ( $b << 5 ) | $s ); }
        method store_mem_disp_byte( $base, $disp, $src ) { my $b = $self->reg($base); my $s = $self->reg($src); $code .= pack( 'L<', 0x39000000 | ( $disp << 10 ) | ( $b << 5 ) | $s ); }
        method lea_rva( $reg, $trva, $txtrva ) { my $r = $self->reg($reg); my $off = $trva - ( $txtrva + length($code) ); my $lo = $off & 0x3; my $hi = ( $off >> 2 ) & 0x7FFFF; $code .= pack( 'L<', 0x10000000 | ( $lo << 29 ) | ( $hi << 5 ) | $r ); }
        ...
        method jcc( $cc, $l ) { push @fixups, { offset => length($code), target => $l, type => 'cond', cc => $cc }; $code .= pack( 'L<', 0x54000000 | $cc ); }
        ...
        method resolve { for (@fixups) { my $t = $labels{ $_->{target} }; my $off = ( $t - $_->{offset} ) / 4; my $instr = unpack( 'L<', substr( $code, $_->{offset}, 4 ) ); if ( $_->{type} eq 'cond' ) { $instr |= ( $off & 0x7FFFF ) << 5 } else { $instr |= ( $off & 0x3FFFFFF ) } substr( $code, $_->{offset}, 4, pack( 'L<', $instr ) ); } }
        method call_rva( $trva, $txtrva ) { my $off = $trva - ( $txtrva + length($code) ); my $lo = $off & 0x3; my $hi = ( $off >> 2 ) & 0x7FFFF; $code .= pack( 'L<', 0x10000000 | ( $lo << 29 ) | ( $hi << 5 ) | 16 ); $code .= pack( 'L<', 0xF9400210 ); $code .= pack( 'L<', 0xD63F0200 ); }
    }

    class Brocken::Emit::X64 {
        field $code : reader = ''; field %labels; field @fixups;
        method reg($r) {
            state $MAP = { rax => 0, rcx => 1, rdx => 2, rbx => 3, rsp => 4, rbp => 5, rsi => 6, rdi => 7, r8  => 8, r9  => 9, r10 => 10, r11 => 11, r12 => 12, r13 => 13, r14 => 14, r15 => 15 };
            return $r if defined($r) && $r =~ /^\d+$/; my $name = lc( $r // '' ); $name =~ s/^\s+|\s+$//g; die "Unknown X64 register: '$r'" unless exists $MAP->{$name}; return $MAP->{$name};
        }
        method _rex( $w, $r, $x, $b ) {
            my $ri = $self->reg($r); my $xi = $self->reg($x); my $bi = $self->reg($b); my $rex = 0x40;
            if ($w) { $rex |= 0x08; } if ( $ri >= 8 ) { $rex |= 0x04; } if ( $xi >= 8 ) { $rex |= 0x02; } if ( $bi >= 8 ) { $rex |= 0x01; }
            if ( !$w && ( ( $ri >= 4 && $ri <= 7 ) || ( $bi >= 4 && $bi <= 7 ) ) ) { return pack( 'C', $rex ); }
            return ( $rex == 0x40 && !$w ) ? '' : pack( 'C', $rex );
        }
        method _emit_modrm( $opcode, $reg, $base, $disp, $w = 1, $prefix = '' ) {
            my $ri = $self->reg($reg); my $bi = $self->reg($base); my $mod = ( $disp == 0 && ( $bi & 7 ) != 5 ) ? 0 : ( $disp >= -128 && $disp <= 127 ? 1 : 2 );
            $code .= $self->_rex( $w, $ri, 0, $bi ) . $prefix . pack( 'C', $opcode ) . pack( 'C', ( $mod << 6 ) | ( ( $ri & 7 ) << 3 ) | ( $bi & 7 ) );
            if ( ( $bi & 7 ) == 4 ) { $code .= pack( 'C', 0x24 ); }
            if ( $mod == 1 ) { $code .= pack( 'c', $disp ); } elsif ( $mod == 2 || ( $mod == 0 && ( $bi & 7 ) == 5 ) ) { $code .= pack( 'l<', $disp ); }
        }
        method append_code($bin) { $code .= $bin }
        ...
        method push_reg($r) { my $ri = $self->reg($r); if ( $ri >= 8 ) { $code .= pack( 'CC', 0x41, 0x50 | ( $ri & 7 ) ); } else { $code .= pack( 'C', 0x50 | $ri ); } }
        method pop_reg($r) { my $ri = $self->reg($r); if ( $ri >= 8 ) { $code .= pack( 'CC', 0x41, 0x58 | ( $ri & 7 ) ); } else { $code .= pack( 'C', 0x58 | $ri ); } }
        ...
        method cmp_reg_imm_32( $r, $imm ) { my $ri = $self->reg($r); $code .= $self->_rex( 0, 0, 0, $ri ) . pack( 'CCl<', 0x81, 0xF8 | ( $ri & 7 ), $imm ); }
        method setcc( $cc, $r ) { my $ri = $self->reg($r); $code .= pack( 'C', 0x40 | ( $ri >= 8 ? 1 : 0 ) ) . pack( 'CCC', 0x0F, $cc, 0xC0 | ( $ri & 7 ) ); }
        ...
        method lea_rva( $reg, $target, $txtrva = 0 ) {
            my $ri = $self->reg($reg);
            if ( $target =~ /^[A-Z_]/i ) {
                $code .= $self->_rex( 1, $ri, 0, 0 ) . pack( 'CC', 0x8D, 0x05 | ( ( $ri & 7 ) << 3 ) ); push @fixups, { offset => length($code), target => $target }; $code .= pack( 'L<', 0 );
            } else {
                my $next = $txtrva + length($code) + 7; $code .= $self->_rex( 1, $ri, 0, 0 ) . pack( 'CC l<', 0x8D, 0x05 | ( ( $ri & 7 ) << 3 ), $target - $next );
            }
        }
