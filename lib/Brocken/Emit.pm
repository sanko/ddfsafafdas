package Brocken::Emit {
    ...
    class Brocken::Emit::ARM64 {
        our %REG = (
            ...
            x24 => 24, x25 => 25, x26 => 26, x27 => 27, x28 => 28, x29 => 29, x30 => 30, sp  => 31, xzr => 31
        );
        field $code : reader = '';
        field %labels;
        field @fixups;
        ...
        method mov_imm( $r, $imm ) {
            my $ri = $self->reg($r);
            # Note: simplified mov_imm for placeholder
            $code .= pack( 'L<', 0xD2800000 | ( ( $imm & 0xFFFF ) << 5 ) | $ri );
        }
    ...
    class Brocken::Emit::X64 {
        ...
        method reg($r) {
            ...
            my $name = lc( $r // '' ); $name =~ s/^\s+|\s+$//g;
            die "Unknown X64 register: '$r'" unless exists $MAP->{$name};
            return $MAP->{$name};
        }
        method _rex( $w, $r, $x, $b ) {
            my $ri  = $self->reg($r); my $xi  = $self->reg($x); my $bi  = $self->reg($b);
            my $rex = 0x40;
            if ($w)         { $rex |= 0x08; }
            if ( $ri >= 8 ) { $rex |= 0x04; }
            if ( $xi >= 8 ) { $rex |= 0x02; }
        ...
        method _emit_modrm( $opcode, $reg, $base, $disp, $w = 1, $prefix = '' ) {
            my $ri  = $self->reg($reg); my $bi  = $self->reg($base);
        ...
        method append_code($bin) { $code .= $bin }
        method mov_reg( $d, $s ) {
            my $di = $self->reg($d); my $si = $self->reg($s);
        ...
        method test_reg_reg( $l, $r ) {
            my $li = $self->reg($l); my $ri = $self->reg($r);
