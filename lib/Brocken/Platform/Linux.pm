...
class Brocken::Platform::Linux : isa(Brocken::Platform) {
    ...
    method emit_intrinsic($target, $as, $inst, $reg_map, $driver) {
        ...
        if ($op eq 'intrinsic_alloc') {
            ...
            $as->mov_reg('rsi', $p); $as->load_reg_mem('rdx', 'rsi', 0);
            $as->add_imm('rsi', 24); $as->mov_imm('rdi', 1); $as->mov_imm('rax', 1); $as->syscall();
        }
            ...
            $as->mov_imm('rax', 1); $as->mov_imm('rdi', 1); $as->lea_reg_disp('rsi', 'rsp', 48); $as->mov_imm('rdx', 1); $as->syscall();
        }
            ...
            $as->mov_reg('rax', 'rsi'); $as->mov_reg('r10', 'rdi');
