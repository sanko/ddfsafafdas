package Brocken::Compiler::Lowering {
    ...
    class Brocken::Compiler::Lowering {
        ...
        method _generate_export_thunk($node) {
            ...
            # We must save RDI, RSI, etc. to the stack before calling M_runtime_init
            my @saved_slots;
            my $arg_idx = 0;
            for my $p ( @{ $node->params } ) {
                ...
                # Allocate a local slot and move the raw ABI register into it
            ...
            # Now it is safe to clobber registers
            $builder->emit( 'call_func', 'void', ['M_runtime_init'] );

            # Ensure context register (R14) is set from the global Isolate pointer
        ...
        method _emit_runtime_init_sub() {
            ...
            # 1. Access the global Isolate pointer slot in the data segment
            my $giso_ptr = $builder->emit( 'load_data_addr', 'ptr', [ $driver->global_iso_offset ] );
            my $l_done   = $builder->new_label();
            my $l_init   = $builder->new_label();

            # 2. Idempotency check: If [global_iso] != 0, skip allocation and jump to sync
            ...
            # --- START OF ONE-TIME INITIALIZATION ---
            $builder->emit_label($l_init);

            # A. Allocate and activate the Isolate
            ...
            # B. Initialize Exception Table Pointer
            # During Codegen, an offset was stored in exception_table_offset.
            # We convert: Absolute Address = Data Segment Base + Offset.
            ...
            # C. Allocate Mark Stack (used by GC)
            ...
            # D. Allocate Initial Heap Block (64KB aligned)
            ...
            # G. Initialize Shadow Stack for Root Tracking
            my $sh = $builder->emit( 'intrinsic_alloc', 'ptr', [1048576] );
            ...
            # --- SYNC / RESUME LOGIC (Runs every call) ---
            $builder->emit_label($l_done);

            # 3. Ensure the context register is set from the global slot
            # (In DLLs, the CPU register R14/X28 is lost between FFI calls)
            my $active_iso = $builder->emit( 'load_mem_disp', 'i64', [ $giso_ptr, 0 ] );
            $builder->emit( 'set_isolate_ctx', 'void', [$active_iso] );

            # 4. Update the Stack Base for the current fiber
            # This anchors the GC/Unwinder to the Perl caller's frame
