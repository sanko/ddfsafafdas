package Brocken::Compiler::Lowering {
    ...
    class Brocken::Compiler::Lowering {
        ...
        method _lower_fiber_new_runtime() {
            ...
            $builder->emit( 'shadow_push', 'void', [$fb] );
            ...
            $builder->emit( 'store_mem_disp', 'void',[ $rb, $driver->context_offset( ( $driver->arch eq 'x64' ? 'r14' : 'x28' ) ), $builder->emit( 'get_isolate_ctx', 'ptr', [] ) ] );
            ...
            my $sh       = $builder->emit( 'call_func', 'ptr', [ 'M_gc_alloc', $leaf_16k ] );
            ...
            $builder->emit( 'store_mem_disp', 'void',[ $cf, $driver->fcb_offset('next'), $builder->emit( 'load_mem_disp', 'ptr',[ $is, $driver->iso_offset('fiber_head') ] ) ] );
        ...
        method lower_program($nodes) {
            ...
            my $init_heap = $builder->emit( 'intrinsic_alloc', 'ptr', [$c64k] );
            ...
            for my $cname ( sort keys %class_info ) {
                ...
                for ( my $i = 0; $i < $ptr_count; $i++ ) {
                    $builder->emit( 'store_mem_disp', 'void',[ $method_base, -16 - ( $i * 8 ), $builder->emit( 'constant', 'i64', [ $c->{ptr_offsets}[$i] ] ) ] );
                }
        ...
        method inject_runtime() {
            ...
            # --- [1] GC Marking ---
            {
                ...
                # Mark as visited in this cycle
                my $mask_val   = $builder->emit( 'constant', 'i64',[ hex("FF000000FFFFFFFF") ] );
                my $new_header = $builder->emit( 'or', 'i64', [ $builder->emit( 'and', 'i64', [ $header, $mask_val ] ), $builder->emit( 'shl', 'i64', [ $cycle, 32 ] ) ] );
                ...
                $builder->emit_cond_br( $builder->emit( 'cmp_lt', 'Int',[ $obj, $heap_base ] ), $l_trace_check, $l_immix );

                $builder->emit_label($l_immix);
                my $block_mask = $builder->emit( 'constant', 'i64', [ -$BLOCK_SIZE ] );
                my $block      = $builder->emit( 'and',      'i64', [ $obj, $block_mask ] );

                # --- Multi-line Marking Loop ---
                # Mark every line covered by this object so the sweeper skips the whole span.
                my $start_line = $builder->emit( 'div', 'i64',[ $builder->emit( 'sub', 'i64', [ $obj, $block ] ), $LINE_SIZE ] );
                my $obj_raw_sz = $builder->emit( 'and', 'i64',[ $header, $builder->emit('constant', 'i64', [hex("1FFFFFFFFFFFFFFF")]) ] );
                # num_lines = (obj_raw_sz + 8 (header) + 127) / 128
                my $num_lines  = $builder->emit( 'div', 'i64', [ $builder->emit( 'add', 'i64',[ $obj_raw_sz, 135 ] ), $LINE_SIZE ] );

                my $mi_slot = $driver->alloc_local_slot();
                $builder->emit('local_store', 'void',[$mi_slot, $builder->emit('constant', 'i64', [0])]);
                my $l_ml = $builder->new_label();
                my $l_md = $builder->new_label();
                $builder->emit_label($l_ml);
                my $mi = $builder->emit('local_load', 'i64', [$mi_slot]);
                $builder->emit_cond_br($builder->emit('cmp_lt', 'Int',[$mi, $num_lines]), $builder->new_label(), $l_md);
                $builder->emit_label($builder->last_instruction->{true_l});
                # Bitmap is at offset 8.
                $builder->emit('store_mem_byte', 'void',[$block, $builder->emit('add', 'i64', [$builder->emit('add', 'i64', [$start_line, $mi]), 8]), 1]);
                $builder->emit('local_store', 'void',[$mi_slot, $builder->emit('add', 'i64', [$mi, 1])]);
                ...
                my $a_el = $builder->emit(
                    'load_mem_disp',
                    'ptr',[   $builder->emit( 'add', 'ptr', [ $obj, $builder->emit( 'add', 'i64',[ 8, $builder->emit( 'mul', 'i64', [ $ca, 8 ] ) ] ) ] ),
                        0
                    ]
                );
                ...
                $builder->emit( 'call_func', 'void',[ 'M_gc_mark_obj', $builder->emit( 'load_mem_disp', 'ptr', [ $builder->emit( 'add', 'ptr', [ $obj, $fo ] ), 0 ] ) ] );
            ...
            # --- [2] GC Sweep (Line-level) ---
            {
                ...
                # Start at line 8 (offset 1024) to skip block metadata
                $builder->emit( 'local_store', 'void',[ $line_idx_slot, $builder->emit( 'constant', 'i64', [8] ) ] );
                ...
                # Sweep all 512 lines
                my $cmp_max  = $builder->emit( 'cmp_lt',     'Int',[ $line_idx, $builder->emit( 'constant', 'i64', [$LINE_COUNT] ) ] );
                my $l_lcont  = $builder->new_label();
                $builder->emit_cond_br( $cmp_max, $l_lcont, $l_lend );
                ...
                $builder->emit_label( $builder->last_instruction->{false_l} );
                my $line_mark  = $builder->emit( 'load_mem_byte', 'Int',[ $curr_bh,   $builder->emit('add', 'i64', [8, $line_idx]) ] );
                ...
                $builder->emit( 'store_mem_byte', 'void',[ $curr_bh, $builder->emit('add', 'i64', [8, $line_idx]), $builder->emit( 'constant', 'i64',[0] ) ] );
                $builder->emit_jump($l_lloop);
            ...
            # ---[3] GC Collect (Root Walking) ---
            {
                ...
                # Clear the mark bitmap (512 lines = 512 bytes)
                # Bitmap starts at offset 8. Clear exactly 512 bytes (64 stores of 8 bytes).
                for ( my $off = 8; $off < 520; $off += 8 ) {
                    $builder->emit( 'store_mem_disp', 'void',[ $curr_bh, $off, $builder->emit( 'constant', 'i64', [0] ) ] );
                }
                # Move to next block.
                my $next_block = $builder->emit( 'load_mem_disp', 'ptr', [ $curr_bh, 0 ] );
                $builder->emit( 'local_store', 'void', [ $bh_slot, $next_block ] );
                ...
                my $sbase   = $builder->emit( 'load_mem_disp', 'ptr',[ $fib, $driver->fcb_offset('shadow_base') ] );
                $builder->emit( 'call_func', 'void', [ 'M_gc_mark_obj', $sbase ] );
                $builder->emit( 'local_store', 'void', [ $bs_slot, $sbase ] );
                $builder->emit( 'local_store', 'void',[ $ps_slot, $builder->emit( 'load_mem_disp', 'ptr',[ $fib, $driver->fcb_offset('shadow_ptr') ] ) ] );
            ...
            # --- [4] Immix Allocator ---
            {
                ...
                $builder->emit( 'leave_func', 'void', [$obj_hdr] );
            }
            ...
            # --- [5] Printers ---
            {
                ...
                $builder->emit(
                    'store_mem_byte',
                    'void',[   $builder->emit( 'local_load', 'ptr', [$bs] ),
                ...
                $builder->emit( 'intrinsic_print_char', 'void',[ $builder->emit( 'load_mem_byte', 'Int', [ $builder->emit( 'local_load', 'ptr', [$bs] ), $ci ] ) ] );
            ...
            # --- [8] Any to String ---
            {
                ...
                $builder->emit(
                    'store_mem_byte',
                    'void',[   $builder->emit( 'local_load',    'ptr', [$ns_slot] ),
        ...
        method lower_Return($node) {
            ...
            if ( $routine_types[-1] eq 'fiber' ) {
                ...
                $builder->emit( 'call_func', 'Any',[ 'M_fiber_switch', $builder->emit( 'load_mem_disp', 'ptr',[ $fcb, $driver->fcb_offset('caller') ] ), $rv ] );
        ...
        method lower_FiberBlock($node) {
            ...
            $self->_emit_all_defers();
            my $fcb = $builder->emit( 'load_iso_disp', 'ptr',[ $driver->iso_offset('current_fcb') ] );
            $builder->emit( 'call_func', 'Any',[ 'M_fiber_switch', $builder->emit( 'load_mem_disp', 'ptr',[ $fcb, $driver->fcb_offset('caller') ] ), $res // 3 ] );
            $builder->emit( 'intrinsic_exit', 'void', [0] );
            $self->_flush_func_locals();
            pop @routine_types;
            $routine_depth--;
