package Brocken::Compiler::Lowering {
    ...
    class Brocken::Compiler::Lowering {
        ...
        field $driver       : param; # The main Brocken::Compiler instance
        ...
        method inject_runtime() {
            ...
            $builder->emit( 'store_iso_disp', 'void', [ $driver->iso_offset('heap_limit'), $new_limit ] );
            ...
            $builder->emit( 'enter_func', 'void', [] );
            my $trans_res = $builder->emit( 'fiber_transfer', 'Any', [ $builder->emit( 'get_arg', 'ptr', [0] ), $builder->emit( 'get_arg', 'ptr', [1] ) ] );
        ...
        method lower_program($nodes) {
            ...
            $builder->emit( 'store_iso_disp', 'void', [ $driver->iso_offset('state_ptr'),  $builder->emit( 'sys_alloc', 'ptr', [$c1m] ) ] );
        ...
        method lower($node) {
            ...
            if ( $node isa Brocken::AST::FiberBlock ) {
                ...
                $routine_depth--;
                $current_scope = $current_scope->parent;
                my $curr   = $builder->emit( 'load_iso_disp', 'ptr', [ $driver->iso_offset('current_fcb') ] );
                my $caller = $builder->emit( 'load_mem_disp', 'ptr', [ $curr, $driver->fcb_offset('caller') ] );
