...
class Brocken::Compiler::Lowering {
    ...
    method inject_runtime_gc_alloc() {
        ...
        $builder->emit(
            ...
            [   $fhdr_slot,
                $builder->emit(
                    'or', 'i64', [ $hdr, $builder->emit( 'and', 'i64', [ $psz, $builder->emit( 'constant', 'i64', [ hex("C000000000000000") ] ) ] ) ]
                )
            ]
        );
        ...
        $builder->emit( 'store_mem_disp', 'void',
            [ $builder->emit( 'local_load', 'ptr', [$fr_slot] ), 0, $builder->emit( 'load_iso_disp', 'ptr', [ $driver->iso_offset('heap_base') ] ) ]
        );
        ...
        $builder->emit_cond_br(
            $builder->emit( 'cmp_lt', 'Int', [ $cmz, $builder->emit( 'add', 'ptr', [ $builder->emit( 'local_load', 'ptr', [$fr_slot] ), 1024 ] ) ] ),
            $l_mzl_body, $l_mze
        );
    ...
    method inject_runtime_print_any() {
        ...
        $builder->emit_cond_br( $builder->emit( 'cmp_eq', 'Int', [ $v, 0 ] ), $l_undef, $l_not_undef );
    ...
    method inject_runtime_str_eq() {
        ...
        # Fast path: If pointers are exactly the same, strings are equal
        ...
        # Pointers to actual UTF-8 payload (offset 16)
    ...
    method inject_runtime_hash_djb2() {
        ...
        # hash = ((hash << 5) + hash) + c
        my $h_shl = $builder->emit( 'shl', 'i64', [ $curr_hash, $builder->emit( 'constant', 'i64', [5] ) ] );
    ...
    method inject_runtime_hash_new() {
        ...
        # 1. Allocate Hash Base (32 bytes)
        my $hash = $builder->emit( 'call_func', 'ptr', [ 'M_gc_alloc', $builder->emit( 'constant', 'i64', [32] ) ] );
        $builder->emit( 'shadow_push', 'void', [$hash] );

        # Offset 0: Tag (1 pointer << 2 | 3 = 7)
        $builder->emit( 'store_mem_disp', 'void', [ $hash, 0, $builder->emit( 'constant', 'i64', [7] ) ] );

        # 2. Allocate Entries Array (8 slots * 16 bytes = 128 bytes + 8 header = 136)
        my $entries = $builder->emit( 'call_func', 'ptr', [ 'M_gc_alloc', $builder->emit( 'constant', 'i64', [136] ) ] );

        # Entries Tag = (16 items << 2) | 1 = 65
        $builder->emit( 'store_mem_disp', 'void', [ $entries, 0, $builder->emit( 'constant', 'i64', [65] ) ] );

        # 3. Store fields
        $builder->emit( 'store_mem_disp', 'void', [ $hash, 8,  $entries ] );                                    # GC scans this!
        $builder->emit( 'store_mem_disp', 'void', [ $hash, 16, $builder->emit( 'constant', 'i64', [0] ) ] );    # Count
        $builder->emit( 'store_mem_disp', 'void', [ $hash, 24, $builder->emit( 'constant', 'i64', [8] ) ] );    # Capacity
        $builder->emit( 'shadow_pop',     'void', [] );
    ...
    method inject_runtime_hash_lookup_slot() {
        ...
        # Allocate slots to protect across M_str_eq calls
        ...
        # If tombstone (k==2), skip string comparison
    ...
    method inject_runtime_hash_lookup() {
        ...
        $builder->emit_label($l_fail);
        $builder->emit( 'leave_func', 'Any', [ $builder->emit( 'constant', 'i64', [0] ) ] );
    }
    ...
    method inject_runtime_hash_exists() {
        ...
        $builder->emit_cond_br( $builder->emit( 'cmp_eq', 'Int', [ $val, $builder->emit( 'constant', 'i64', [2] ) ] ),
            $l_fail, $builder->new_label() );
        $builder->emit_label( $builder->last_instruction->{false_l} );
        $builder->emit( 'leave_func', 'i64', [ $builder->emit( 'constant', 'i64', [3] ) ] );    # 3 is (1 << 1) | 1 (True)
        $builder->emit_label($l_fail);
        $builder->emit( 'leave_func', 'i64', [ $builder->emit( 'constant', 'i64', [1] ) ] );    # 1 is (0 << 1) | 1 (False)
    }
    ...
    method inject_runtime_hash_delete() {
        ...
        # Set tombstone value
        $builder->emit( 'store_mem_disp', 'void', [ $slot, 8, $builder->emit( 'constant', 'i64', [2] ) ] );
    ...
    method inject_runtime_hash_insert() {
        ...
        # If it's a key tombstone (2), we could insert here, but we must probe further to ensure
        # the key doesn't actually exist further down. For simplicity, we just skip key tombstones.
        my $l_check_eq = $builder->new_label();
        my $l_next     = $builder->new_label();
        $builder->emit_cond_br( $builder->emit( 'cmp_eq', 'Int', [ $k, $builder->emit( 'constant', 'i64', [2] ) ] ), $l_next, $l_check_eq );
        $builder->emit_label($l_check_eq);
    ...
    method lower_VarDecl($node) {
        ...
        my $sho = undef;
        if ( $ft =~ /^(Any|String|Array|Hash|Tuple|Fiber|Class)$/ || $ft !~ /^(Int|Float|i64|double|ptr|void)$/ ) {
    ...
    method lower_Assignment($node) {
        ...
        # 1. Array/Hash index assignment: $foo[$idx] = $val
        if ( $node->name isa Brocken::AST::Expr::IndexExpr ) {
            ...
            $builder->emit_label( $builder->last_instruction->{true_l} );
            $builder->emit( 'intrinsic_throw', 'void', [ $builder->emit( 'constant', 'i64', [255] ) ] );
            $builder->emit_label($l_not_null);
            ...
            my $offset  = $builder->emit( 'mul', 'i64', [ $raw_idx, 8 ] );
            my $addr    = $builder->emit( 'add', 'ptr', [ $builder->emit( 'add', 'ptr', [ $src_reg, 8 ] ), $offset ] );
            if ( $vt =~ /^(Any|String|Array|Hash|Tuple|Fiber|Class)$/ || $vt !~ /^(Int|Float|i64|double|ptr|void)$/ ) {
                $self->_emit_write_barrier( $addr, 0, $vr );
            }
            else {
                $builder->emit( 'store_mem_disp', 'void', [ $addr, 0, $vr ] );
            }
        ...
        # 2. Field assignment: $obj->x = $val
        if ( $node->name isa Brocken::AST::Expr::MethodCall ) {
            my $mc        = $node->name;
            my $inv       = $mc->object;
        ...
        # 3. Standard local/state variable
        my $s = $current_scope->resolve( $node->name ) // die "Undeclared variable: " . $node->name;
        if ( defined $s->stack_offset && $s->stack_offset < 0 ) {
            my $sl_ptr = $builder->emit( 'local_load', 'ptr', [ $current_scope->resolve('$self')->stack_offset ] );
            if ( $vt =~ /^(Any|String|Array|Hash|Tuple|Fiber|Class)$/ || $vt !~ /^(Int|Float|i64|double|ptr|void)$/ ) {
    ...
    method lower_BinOp($node) {
        ...
        if ( exists $scm->{ $node->op } ) {
            my $lc      = $lt eq 'String' ? $lr : $builder->emit( 'call_func', 'ptr', [ 'M_any_to_str', $lr ] );
            my $rc      = $rt eq 'String' ? $rr : $builder->emit( 'call_func', 'ptr', [ 'M_any_to_str', $rr ] );
            my $cmp_res = $builder->emit( 'call_func', 'i64', [ 'M_str_cmp', $lc, $rc ] );
    ...
    method lower_Call($node) {
        ...
        if ( $node->name eq 'keys' ) {
            my ( $r, $t ) = $self->lower( $node->args->[0] );
            return ( $builder->emit( 'call_func', 'ptr', [ 'M_hash_keys', $r ] ), 'Array' );
        }
        if ( $node->name eq 'values' ) {
            my ( $r, $t ) = $self->lower( $node->args->[0] );
            return ( $builder->emit( 'call_func', 'ptr', [ 'M_hash_values', $r ] ), 'Array' );
        }
    ...
    method lower_MethodCall($node) {
        ...
        $builder->emit_label($l_not_null);
        my $vtp = $builder->emit( 'load_mem_disp', 'ptr', [ $or,  0 ] );
        my $fn  = $builder->emit( 'load_mem_disp', 'ptr', [ $vtp, ( $global_methods{$mn} // die "Unknown method '$mn' called on instance" ) * 8 ] );
    ...
    method lower_TupleLiteral($node) {
        my $ct  = scalar @{ $node->elements };
        my $sz  = 8 + ( $ct * 8 );
        my $tup = $builder->emit( 'call_func', 'ptr', [ 'M_gc_alloc', $builder->emit( 'constant', 'i64', [$sz] ) ] );
        ...
        for my $el ( @{ $node->elements } ) {
            ...
            if ( $vt =~ /^(Any|String|Array|Hash|Tuple|Fiber|Class)$/ || $vt !~ /^(Int|Float|i64|double|ptr|void)$/ ) {
                ...
                $builder->emit( 'store_mem_disp', 'void', [ $tup, $off, $vr ] );
            }
        }
        $builder->emit( 'shadow_pop', 'void', [] );
        return ( $tup, 'Tuple' );
    ...
    method lower_HashLiteral($node) {
        ...
        for my $pair ( @{ $node->pairs } ) {
            ...
            my $k_str = ( $kt eq 'String' ) ? $kr : $builder->emit( 'call_func', 'ptr', [ 'M_any_to_str', $kr ] );
            $builder->emit( 'call_func', 'void', [ 'M_hash_insert', $hash, $k_str, $vr ] );
            $builder->emit( 'shadow_pop', 'void', [] );    # pop kr
        }
        $builder->emit( 'shadow_pop', 'void', [] );        # pop hash
        return ( $hash, 'Hash' );
    }
    ...
    method lower_IndexExpr($node) {
        ...
        # Read the tag at offset 0
        my $tag_word = $builder->emit( 'load_mem_disp', 'i64', [ $src_reg,  0 ] );
        my $type_tag = $builder->emit( 'and',           'i64', [ $tag_word, 3 ] );    # Get lowest 2 bits
        ...
        my $offset  = $builder->emit( 'mul', 'i64', [ $raw_idx, 8 ] );
        my $addr    = $builder->emit( 'add', 'ptr', [ $builder->emit( 'add', 'ptr', [ $src_reg, 8 ] ), $offset ] );
        my $val     = $builder->emit( 'load_mem_disp', 'Any', [ $addr, 0 ] );
        $builder->emit( 'local_store', 'void', [ $res_slot, $val ] );
        $builder->emit_jump($l_end);

        # --- HASH PATH ---
        $builder->emit_label($l_is_hash);
        my $k_str = ( $idx_type eq 'String' ) ? $idx_reg : $builder->emit( 'call_func', 'ptr', [ 'M_any_to_str', $idx_reg ] );
        my $h_val = $builder->emit( 'call_func', 'Any', [ 'M_hash_lookup', $src_reg, $k_str ] );
        $builder->emit( 'local_store', 'void', [ $res_slot, $h_val ] );
        $builder->emit_jump($l_end);
        $builder->emit_label($l_end);
        $builder->emit( 'shadow_pop', 'void', [] );
        return ( $builder->emit( 'local_load', 'Any', [$res_slot] ), 'Any' );
    ...
    method lower_Eval($node) {
        die "Eval is currently disabled while the JIT engine is being redesigned.\n";
    ...
    method lower_ClassDecl($node) {
        ...
        for my $f ( @{ $node->fields } ) {
            ( my $clean_name = $f->name ) =~ s/^[\$@%]//;    # Strip sigil
            ...
            $builder->emit( 'intrinsic_throw', 'void', [ $builder->emit( 'constant', 'i64', [255] ) ] );
            $builder->emit_label($l_not_null);
            my $val = $builder->emit( 'load_mem_disp', 'Any', [ $self_ptr, $field_offset ] );
            $builder->emit( 'leave_func', 'Any', [$val] );
            ...
            if ( $f->type =~ /^(Any|String|Array|Hash|Tuple|Fiber|Class)$/ || $f->type !~ /^(Int|Float|i64|double|ptr|void)$/ ) {
                $self->_emit_write_barrier( $self_ptr, $field_offset, $new_val );
            }
            else {
                $builder->emit( 'store_mem_disp', 'void', [ $self_ptr, $field_offset, $new_val ] );
            }
            $builder->emit( 'leave_func', 'void', [] );
            $field_offset += 8;
        }
        ...
        for my $m ( @{ $node->methods } ) {
            ...
            # Provide direct local scope bindings to class fields inside methods
    ...
    method register_classes($nodes) {
        for my $node (@$nodes) {
            if ( $node isa Brocken::AST::OOP::ClassDecl ) {
                ...
                for my $f ( @{ $node->fields } ) {
                    push @po, $co if $f->type =~ /^(Any|String|Array|Hash|Tuple|Fiber|Class)$/;
                    ( my $clean_name = $f->name ) =~ s/^[\$@%]//;

                    # Register the auto-generated accessors into the global method dictionary
                    my $getter = $clean_name;
                    push @mn, $getter;
                    $global_methods{$getter} //= $global_method_count++;
                    my $setter = "set_" . $clean_name;
                    push @mn, $setter;
                    $global_methods{$setter} //= $global_method_count++;
                    $co += 8;
                }
