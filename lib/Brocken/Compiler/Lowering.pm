package Brocken::Compiler::Lowering {
    ...
    class Brocken::Compiler::Lowering {
        ...
        method lower($node) {
            ...
            if ( $node isa Brocken::AST::UnaryOp && $node->op eq '!' ) {
                my ( $r, $t ) = $self->lower( $node->expr );
                return ( $builder->emit( 'cmp_eq', 'Int', [ $r, 0 ] ), 'Int' );
            }
            if ( $node isa Brocken::AST::BinOp ) {
                if ( $node->op eq '&&' || $node->op eq '||' ) {
                    ...
                    # AND: if left is 0, jump to l_short. OR: if left is 1, jump to l_short.
                    if   ($is_and) { $builder->emit_cond_br( $l_reg, $builder->new_label(), $l_short ); }
                    else           { $builder->emit_cond_br( $l_reg, $l_short,              $builder->new_label() ); }
                    ...
                    $builder->emit( 'local_store', 'void', [ $res_slot, $is_and ? 0 : 1 ] );
                    $builder->emit_label($l_end);
            ...
            if ( $node isa Brocken::AST::Ternary ) {
                my $res_slot = $driver->alloc_local_slot();
