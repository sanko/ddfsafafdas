package Brocken::Compiler::Lowering {
    ...
    class Brocken::Compiler::Lowering {
            ...
            for my $node (@$nodes) {
                if ( $node isa Brocken::AST::ClassDecl ) {
                    ...
                    for my $m ( @{ $node->methods } ) {
                        ...
                        if ( !exists $global_methods{ $m->name } ) { $global_methods{ $m->name } = $global_method_count++; }
                    }
        ...
        method inject_runtime() {
            ...
            {
                # Fiber Allocation Block
                # Manually spills intervals aggressively to prevent register allocator overload
                $driver->reset_locals();
                ...
                my $reg_block2 = $builder->emit( 'local_load', 'ptr', [$reg_block_slot] );
                $builder->emit( 'store_mem_disp', 'void',[ $fcb_reg2, $driver->fcb_offset('sp'), $reg_block2 ] );
        ...
        method lower_program($nodes) {
            ...
            while (@fragments) { my $frag = shift @fragments; $builder->push_instructin($_) for @$frag; }
        ...
        method lower($node) {
            ...
            if ( $node isa Brocken::AST::Assignment ) {
                ...
                my $sym = $current_scope->resolve( $node->name ) // die 'Undeclared ' . $node->name . "\n";
                if ( defined $sym->stack_offset && $sym->stack_offset < 0 ) {
                    my $self_sym = $current_scope->resolve('$self') // die "Cannot assign field";
                    my $self_ptr = $builder->emit( 'local_load', 'ptr',[ $self_sym->stack_offset ] );
            ...
            if ( $node isa Brocken::AST::ClassDecl ) {
                ...
                else { my $zero = $builder->emit( 'constant', 'i64', [0] ); $builder->emit( 'store_mem_disp', 'void', [ $obj, 0, $zero ] ); }
            ...
            if ( $node isa Brocken::AST::MethodCall ) {
                ...
                my $gidx     = $global_methods{ $node->name } // die "Compile Error: Method " . $node->name;
            ...
            if ( $node isa Brocken::AST::Call ) {
                ...
                if ( $node->name =~ /^(say|print)$/ ) {
                    ...
                    if    ( $t eq 'String' ) { $builder->emit( 'builtin_print', 'void', [$r] ); }
            ...
            if ( $node isa Brocken::AST::Return ) {
                ...
                else { $builder->emit( 'leave_func', 'void',[$ret_val] ); }
                return ( undef, 'void' );
            }
