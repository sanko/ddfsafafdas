package Brocken::Compiler::Lowering {
    ...
    class Brocken::Compiler::Lowering {
        ...
        method inject_runtime() {
            $driver->reset_locals();
            $builder->emit_label('M_gc_mark_obj'); $builder->emit('enter_func', 'void',[]);
            my $obj_ptr = $builder->emit('get_arg', 'ptr', [0]);
            my $l_null = $builder->new_label(); my $l_not_null = $builder->new_label();
            ...
            my $l_loop_start = $builder->new_label(); my $l_loop_end = $builder->new_label(); my $l_loop_body = $builder->new_label();
            ...
            my $el_off = $builder->emit('add', 'i64',[24, $builder->emit('mul', 'i64',[$arr_idx, 8])]);
            $builder->emit('call_func', 'void',['M_gc_mark_obj', $builder->emit('load_mem_disp', 'ptr',[$builder->emit('add', 'ptr',[$obj_ptr, $el_off]), 0])]);
            $arr_idx = $builder->emit('add', 'i64', [$arr_idx, 1]);
            $builder->emit_jump($l_loop_start);
            $builder->emit_label($l_loop_end); $builder->emit_label($l_null); $builder->emit('leave_func', 'void',[0]);

            $builder->emit_label('M_gc_sweep'); $builder->emit('enter_func', 'void',[]);
            my $limit_reg = $builder->emit('load_iso_disp', 'ptr',[$driver->iso_offset('heap_limit')]);
            $builder->emit('store_iso_disp', 'void',[$driver->iso_offset('heap_ptr'), $builder->emit('sub', 'ptr',[$limit_reg, $builder->emit('constant', 'i64',[1048576])])]);
            $builder->emit('leave_func', 'void', [0]);

            $builder->emit_label('M_gc_collect'); $builder->emit('enter_func', 'void',[]);
            my $fib_head = $builder->emit('load_iso_disp', 'ptr',[32]);
            my $l_fib_loop = $builder->new_label(); my $l_fib_end = $builder->new_label(); my $l_fib_body = $builder->new_label();
            ...
            my $l_shad_loop = $builder->new_label(); my $l_shad_end = $builder->new_label(); my $l_shad_body = $builder->new_label();
            ...
            $driver->reset_locals();
            $builder->emit_label('M_gc_alloc'); $builder->emit( 'enter_func', 'void',[] );
            ...
            $builder->emit( 'store_iso_disp', 'void',[ $driver->iso_offset('heap_limit'), $builder->emit( 'add', 'ptr',[ $new_region, $total_req ] ) ] );
            $builder->emit( 'store_mem_disp', 'void',[ $new_region, 0, $payload_sz ] );
            $builder->emit( 'leave_func',     'void', [$builder->emit('add', 'ptr',[$new_region, 8])] );

            $driver->reset_locals();
            $builder->emit_label('M_print_int'); $builder->emit( 'enter_func', 'void',[] );
            my $n = $builder->emit( 'get_arg', 'i64',[0] );
            my $l_z = $builder->new_label(); my $l_not_z = $builder->new_label();
            $builder->emit_cond_br( $builder->emit( 'cmp_eq', 'Int', [ $n, 0 ] ), $l_z, $l_not_z );
            $builder->emit_label($l_z); $builder->emit( 'builtin_print_char', 'void', [48] ); $builder->emit( 'leave_func', 'void', [0] );
            ...
            $builder->emit_label($l4); $builder->emit( 'leave_func', 'void', [0] );

            $driver->reset_locals();
            $builder->emit_label('M_fiber_switch'); $builder->emit( 'enter_func', 'void',[] );
            $builder->emit( 'leave_func', 'void',[$builder->emit( 'fiber_transfer', 'Any',[ $builder->emit( 'get_arg', 'ptr',[0] ), $builder->emit( 'get_arg', 'ptr', [1] ) ] )] );

            $driver->reset_locals();
            $builder->emit_label('M_fiber_new'); $builder->emit( 'enter_func', 'void',[] );
            ...
            $builder->emit( 'store_mem_disp', 'void',[ $fcb_reg, $driver->fcb_offset('stack_base'), $mstack ] );
        ...
        method lower_program($nodes) {
            ...
            $builder->emit( 'store_iso_disp', 'void',[ 16, $builder->emit('sys_alloc', 'ptr', [$c1m]) ] );
            ...
            $builder->emit( 'store_mem_disp', 'void',[ $main_fcb, 40, $zero ] );
            $builder->emit( 'store_mem_disp', 'void',[ $iso_reg, 32, $main_fcb ] );
        ...
        method lower($node) {
            ...
            if ( $node isa Brocken::AST::StateDecl ) {
                ...
                my $l_init = $builder->new_label(); my $l_done = $builder->new_label();
                my $sb     = $builder->emit( 'load_iso_disp', 'ptr',[ $driver->iso_offset('state_ptr') ] );
            ...
            if ( $node isa Brocken::AST::Assignment ) {
                ...
                if ( $sym->is_state ) {
                    $builder->emit( 'store_mem_disp', 'void', [ $builder->emit( 'load_iso_disp', 'ptr',[ $driver->iso_offset('state_ptr') ] ), 4096 + ( $sym->state_idx * 8 ), $v_reg ] );
                    return ( $v_reg, $sym->type );
                }
            ...
            if ( $node isa Brocken::AST::MethodCall ) {
                ...
                if ($node->name eq 'new') {
                    my $ptr = $builder->emit('call_func', 'ptr', ['M_gc_alloc', 64]);
                    $builder->emit('shadow_push', 'void', [$ptr]);
                    return ($ptr, $obj_typ);
                }
                my @args = map { ($self->lower($_))[0] } @{$node->args};
                return ($builder->emit('call_func', 'i64',["M_$obj_typ\::" . $node->name, $obj_reg, @args]), 'Int');
            }
            ...
            if ( $node isa Brocken::AST::FiberBlock ) {
                my $fib_label  = $builder->new_label(); my $skip_label = $builder->new_label();
                $builder->emit_jump($skip_label);
                my @main_instructions = $builder->instructions; $builder->set_instructions();
                my $saved_local_ptr = $driver->local_ptr; $driver->reset_locals();
                $builder->emit_label($fib_label); $builder->emit( 'enter_func', 'void',[] );
                ...
                $builder->emit( 'fiber_transfer', 'Any',[ $builder->emit( 'load_mem_disp', 'ptr',[ $builder->emit( 'load_iso_disp', 'ptr',[ $driver->iso_offset('current_fcb') ] ), $driver->fcb_offset('caller') ] ), $res // 0 ] );
                $builder->emit( 'exit_program', 'void', [0] );
                my @fiber_instructions = $builder->instructions; $builder->set_instructions( @main_instructions, @fiber_instructions );
                $builder->emit_label($skip_label); $driver->set_local_ptr($saved_local_ptr);
                return ( $builder->emit( 'call_func', 'ptr',[ 'M_fiber_new', $fib_label ] ), 'Fiber' );
            }
            if ( $node isa Brocken::AST::Yield ) {
                return ( $builder->emit( 'call_func', 'Int',[ 'M_fiber_switch', $builder->emit( 'load_mem_disp', 'ptr',[ $builder->emit( 'load_iso_disp', 'ptr', [ $driver->iso_offset('current_fcb') ] ), $driver->fcb_offset('caller') ] ), ($self->lower( $node->expr ))[0] ] ), 'Int' );
            }
            if ( $node isa Brocken::AST::Call ) {
                if ( $node->name eq 'transfer' ) { return ( $builder->emit( 'call_func', 'Int',[ 'M_fiber_switch', ($self->lower( $node->args->[0] ))[0], ($self->lower( $node->args->[1] ))[0] ] ), 'Int' ); }
                if ( $node->name =~ /^(say|print)$/ ) {
                    ...
                    if ( $node->name eq 'say' ) { $builder->emit( 'builtin_print', 'void',[ $builder->emit( 'load_data_addr', 'ptr',[ $data_segment->add_string("\n") ] ) ] ); }
                    return ( undef, 'void' );
                }
                my @args = map { ( $self->lower($_) )[0] } @{ $node->args };
            ...
            if ( $node isa Brocken::AST::Return ) {
                ...
                $builder->emit( 'leave_func', 'void', [($self->lower( $node->expr ))[0]] );
                return ( undef, 'void' );
            }
            ...
            if ( $node isa Brocken::AST::If ) {
                my $l_then = $builder->new_label(); my $l_else = $builder->new_label(); my $l_end  = $builder->new_label();
                $builder->emit_cond_br( ($self->lower( $node->condition ))[0], $l_then, $l_else );
                $builder->emit_label($l_then); $self->lower( $node->then_block ); $builder->emit_jump($l_end);
                $builder->emit_label($l_else); $self->lower( $node->else_block ) if $node->else_block; $builder->emit_label($l_end);
                return ( undef, 'void' );
            }
            if ( $node isa Brocken::AST::While ) {
                my $l_start = $builder->new_label(); my $l_body = $builder->new_label(); my $l_end = $builder->new_label();
                $builder->emit_label($l_start);
                $builder->emit_cond_br( ($self->lower( $node->condition ))[0], $l_body, $l_end );
                $builder->emit_label($l_body); $self->lower( $node->body ); $builder->emit_jump($l_start);
                $builder->emit_label($l_end);
                return ( undef, 'void' );
            }
            if ( $node isa Brocken::AST::ArrayLiteral ) {
                my $count = scalar @{ $node->elements }; my $size = 24 + ( $count * 8 );
                ...
                for my $el ( @{ $node->elements } ) {
                    $builder->emit( 'store_mem_disp', 'void',[ $arr_ptr, 24 + ( $idx++ * 8 ), ($self->lower($el))[0] ] );
                }
