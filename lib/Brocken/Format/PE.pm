package Brocken::Format::PE {
    ...
    class Brocken::Format::PE : isa(Brocken::Format) {
        ...
        method write_bin( $filename, $text, $data, $arch, $os ) {
            ...
            print $fh pack(
                'S< C C L< L< L< L< L< Q< L< L< S< S< S< S< S< S< L< L< L< L< S< S< Q< Q< Q< Q< L< L<', 0x20B, 14, 0, $l->get('.text')->{size},
                $l->get('.data')->{size} + length($idata_raw), 0, $l->get('.text')->{rva}, $l->get('.text')->{rva},   0x140000000,    # ImageBase
                0x1000, 0x200, 6, 0, 0, 0, 6, 0, 0, $image_size, $l->header_size, 0, 3, 0x8140, 0x100000, 0x1000, 0x100000, 0x1000, 0, 16
            );
            ...
            for my $s ( $l->sections ) {
                ...
                my $payload = $s->{name} eq '.text' ? $text :
                    ( $s->{name} eq '.idata' ? $idata_raw : ( $s->{name} eq '.debug' ? ( $dd || "\0" ) : ( $data || "\0" ) ) );
                my $file_size = ( $s->{size} + 0x1FF ) & ~0x1FF;
