    ...
    class Brocken::Format::ELF :isa(Brocken::Format) {
        ...
        method write_bin($f, $text, $data, $arch, $os) {
            ...
            my $ehdr = pack(
                ...
                1, $base + $l->get('.text')->{rva}, 64, 0, 0, 64, 56, 2, 0, 0, 0
            );

            my $ph_t = pack('LL Q Q Q Q Q Q', 1, 5, $l->get('.text')->{off}, $base + $l->get('.text')->{rva},
                            $base + $l->get('.text')->{rva}, $l->get('.text')->{size}, $l->get('.text')->{size}, 0x1000);
            my $ph_d = pack('LL Q Q Q Q Q Q', 1, 6, $l->get('.data')->{off}, $base + $l->get('.data')->{rva},
                            $base + $l->get('.data')->{rva}, $l->get('.data')->{size}, $l->get('.data')->{size}, 0x1000);
