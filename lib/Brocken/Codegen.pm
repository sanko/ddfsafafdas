package Brocken::Codegen {
    ...
    class Brocken::Codegen {
            ...
            for my $inst (@$instructions) {
                ...
                if    ( $op eq 'label' ) { $as->mark_label( $inst->{name} ); }
                    ...
                    if ( $target =~ /^[A-Z_]/i ) {
                        ...
                        $as->lea_rva( $reg_map{ $inst->{dest} }, $driver->data_rva + $target, $driver->text_rva );
                    }
                }
                elsif ( $op eq 'mov' ) {
                    ...
                    if ( $driver->os eq 'win64' ) { $as->mov_reg( 'rcx', $dest ); $as->call_rva( $driver->import_rva('ExitProcess'), $driver->text_rva ); }
                    else                          { $driver->exit_reg($dest); }
                }
                elsif ( $op eq 'map_op' ) {
                    my $d = $reg_map{ $inst->{dest} };
                    ...
                    if ( $driver->os eq 'win64' ) { $as->mov_imm( 'rcx', 1 ); $as->lea_rva( 'rdx', 'M_veh_handler', $driver->text_rva ); $as->call_rva( $driver->import_rva('AddVectoredExceptionHandler'), $driver->text_rva ); }
                    else {
                        ...
                        $as->mov_imm( 'rax', 13 ); $as->mov_imm( 'rdi', 11 ); $as->lea_reg_disp( 'rsi', 'rsp', 32 ); $as->mov_imm( 'rdx', 0 ); $as->mov_imm( 'r10', 8 ); $as->syscall();
                    }
                }
                elsif ( $op eq 'setup_console' ) { if ( $driver->os eq 'win64' ) { $as->mov_imm( 'rcx', 65001 ); $as->call_rva( $driver->import_rva('SetConsoleOutputCP'), $driver->text_rva ); } }
                    ...
                    if ( $driver->os eq 'win64' ) {
                        ...
                        $as->sub_imm( 'rsp', 40 ); $as->mov_reg( 'rcx', 'r8' ); $as->mov_imm( 'rdx', 4096 ); $as->mov_imm( 'r8', 0x1000 ); $as->mov_imm( 'r9', 4 );
                        ...
                        $as->mark_label('M_segv_handler'); $as->load_reg_mem( 'rdi', 'rsi', 16 ); $as->mov_imm( 'r11', -4096 ); $as->append_code( pack( 'CCC', 0x48, 0x21, 0xDF ) );
                        $as->mov_imm( 'rsi', 4096 ); $as->mov_imm( 'rdx', 3 ); $as->mov_imm( 'rax', 10 ); $as->syscall(); $as->mov_imm( 'rax', 15 ); $as->syscall();
                    }
                }
                    ...
                    if ( defined $rv ) { if ( $inst->{args}[0] =~ /^%/ ) { $as->mov_reg( 'rax', $rv ) if $rv ne 'rax'; } else { $as->mov_imm( 'rax', $rv ); } }
        ...
        method _allocate_registers( $live_ref, $driver ) {
            ...
            elsif ( $driver->os eq 'win64' ) { @free = qw(rbx rsi rdi r12 r13 r15); }
            ...
            for my $iv (@intervals) {
                ...
                my $phys = shift @free; die "Out of registers! Needed for $iv->{vreg}" if !defined $phys;
                $rmap{ $iv->{vreg} } = $phys; push @active, { vreg => $iv->{vreg}, phys => $phys, end => $iv->{end} };
