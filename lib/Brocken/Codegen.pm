        ...
        method compile( $instructions, $driver ) {
            ...
            for my $inst (@$instructions) {
                ...
                if    ( $op eq 'label' ) { $as->mark_label( $inst->{name} ); }
                    ...
                    $as->append_code(
                        pack( 'CCCC',
                            0x48 | ( $si_idx >= 8 ? 4 : 0 ) | ( $di_idx >= 8 ? 1 : 0 ),
                            ( $op eq 'and' ? 0x21 : 0x09 ),
                            0xC0 | ( ( $si_idx & 7 ) << 3 ) | ( $di_idx & 7 ) )
                    );
                }
                    ...
                    my $target = ( $arch eq 'x64' ? 'rdi' : 'x0' );
                    if ( $inst->{args}[0] =~ /^%/ ) {
                        $as->mov_reg( $target, $reg_map{ $inst->{args}[0] } ) if $target ne $reg_map{ $inst->{args}[0] };
                    }
                    else { $as->mov_imm( $target, $v // 0 ); }
                    if ( $driver->os eq 'win64' ) {
                        $as->mov_reg( 'rcx', $target );
                    ...
                    for my $i ( 0 .. $#args ) {
                        my $arg     = $args[$i];
                        my $dst_abi = $self->_abi_arg_reg( $i, $driver->os );
