package Brocken::Codegen {
    ...
    class Brocken::Codegen {
        ...
        method compile( $instructions, $driver ) {
            my $target = $driver->target; my $as = $driver->as;
        ...
        method _spill_vreg($insts, $vreg, $driver) {
            my $slot = $driver->alloc_local_slot(); my @new;
            for my $old (@$insts) {
                my %ins = %$old; $ins{args} = [@{$old->{args}}] if $old->{args};
                my ($needs_l, $l_reg, $needs_s, $s_reg) = (0, undef, 0, undef);
                if ($ins{args}) { for (@{$ins{args}}) { if (defined $_ && $_ eq $vreg) { $needs_l = 1; $l_reg = "%S" . ++$spill_count; $_ = $l_reg; } } }
                if ($ins{op} eq 'cond_br' && defined $ins{reg} && $ins{reg} eq $vreg) { $needs_l = 1; $l_reg = "%S" . ++$spill_count; $ins{reg} = $l_reg; }
        ...
        method _allocate_registers( $instructions, $driver ) {
            my $changed = 1; my %rmap; my $safety = 200;
            while ($changed && $safety-- > 0) {
                $changed = 0; my %live = $self->_analyze_liveness($instructions);
                my @free = @{$driver->target->registers()};
                my @intervals = sort { $a->{start} <=> $b->{start} } map { {vreg=>$_, %{$live{$_}}} } keys %live;
                my @active; %rmap = ();
                for my $iv (@intervals) {
                    @active = grep { if ($_->{end} < $iv->{start}) { push @free, $_->{phys}; 0 } else { 1 } } @active;
                    my $phys = shift @free;
                    if (!$phys) {
                        @active = sort { $b->{end} <=> $a->{end} } @active;
                        my $spill = ($active[0] && $active[0]->{end} > $iv->{end}) ? $active[0]->{vreg} : $iv->{vreg};
                        $self->_spill_vreg($instructions, $spill, $driver); $changed = 1; last;
                    }
