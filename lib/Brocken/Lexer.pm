...
no warnings 'portable', 'experimental::class';
    ...
    class Brocken::Lexer {
        field $source : param;
        field $pos = 0; field $line = 1; field $col = 1;
        ...
        method lex() {
            ...
            while ( $pos < $length ) {
                ...
                if ( $remaining =~ /^"([^"\\]*(?:\\.[^"\\]*)*)"/s ) {
                    my $val = $1; $val =~ s/\\n/\n/g; $val =~ s/\\"/"/g;
                    push @tokens, $self->_make_token( 'STRING', $val ); $self->_advance( length($&) ); next;
                }
                if ( $remaining =~ /^([\$@%]?[a-zA-Z_]\w*)/ ) {
                    my $val = $1; my $type = 'IDENT';
                    if ($val =~ /^[\$@%]/) { $type = 'VAR'; } elsif ($KEYWORDS{$val}) { $type = 'KEYWORD'; }
                    push @tokens, $self->_make_token( $type, $val ); $self->_advance( length($val) ); next;
                }
