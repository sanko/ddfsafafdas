...
sub compile_and_run {
    ...
    eval { $compiler->compile_source( $source, $exe ); };
    if ( my $err = $@ ) {
        return ( undef, undef, $err );
    }
    my $output = `"$exe"`;
    my $status = $?;
    return ( $output, $compiler, undef, $status );
}

# Test 1: our with type and initializer
{
    my ( $out, $compiler, $err ) = compile_and_run( source => <<'BROCKEN' );
our Int $x = 42;
say $x;
BROCKEN
    ok !$err, 'our Int $x = 42 compiled' or diag "Error: $err";
    is $out, "42\n", 'our Int $x = 42 prints 42';
}
...
BROCKEN
    ...
    is $out, "42\n", 'our Int $x assigned after decl = 42';
}
...
BROCKEN
    ...
    is $out, "3\n", 'our $counter incremented across sub calls = 3';
}
...
BROCKEN
    ...
    is $out, "10\n20\n", 'our $x accessible inside block and modified';
}
